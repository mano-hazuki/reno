...
export function BackgroundProperty() {
	const [isOpen, setOpen] = useState<boolean>(false);
	const toggle = () => setOpen(!isOpen);

	const [elements, setElements] = useAtom(elementsAtom);
	const elementId = useAtomValue(elementSelectedIdAtom);
	const color = elements.find((e) => e.id === elementId)!.background!.color;
	const updateColor = (event: ChangeEvent<HTMLInputElement>) => {
		...
		setElements((prev) => {
			...
			prev.find((e) => e.id === elementId)!.background!.color = event.target.value;
		});
	};
	return (
		<li key="background" className="w-full h-fit px-4 flex flex-col justify-start items-center gap-2">
			<button className="w-full h-fit p-4 border-b border-b-white border-opacity-5 flex flex-row justify-between items-center" type="button" onClick={() => toggle()}>
				<span className="text-lg text-white text-opacity-80">Background</span>
